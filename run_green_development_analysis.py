...
def main():
    ...
    generate_detailed_report(results, output_dir, positive_cols, negative_cols)
...
def generate_detailed_report(results, output_dir, positive_cols, negative_cols):
    """生成详细的分析报告"""
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("- **经济子系统权重**: {:.4f}\n".format(weight_df[weight_df['指标'].isin(positive_cols[:3])]['权重'].sum()))
        f.write("- **环境子系统权重**: {:.4f}\n\n".format(weight_df[~weight_df['指标'].isin(positive_cols[:3])]['权重'].sum()))
        ...
        for idx, row in top10.iterrows():
            f.write(f"| {idx+1} | {row['省份']} | {row['年份']} | {row['综合得分']:.4f} | {row['经济子系统得分']:.4f} | {row['环境子系统得分']:.4f} |\n")
        ...
        f.write("### 8.1 主要发现\n\n")
        f.write("1. **绿色发展水平**: 我国绿色发展水平整体呈{}趋势，综合得分平均值为{:.4f}\n".format(
            "上升" if yearly_avg['综合得分'].iloc[-1] > yearly_avg['综合得分'].iloc[0] else "下降",
            results['综合得分'].mean()
        ))
        f.write("2. **系统协调**: 经济与环境子系统耦合协调度平均值为{:.4f}，主要处于{}水平\n".format(
            results['耦合协调度'].mean(),
            level_counts.index[0]
        ))
        f.write("3. **区域差异**: 各省份绿色发展水平存在显著差异，{}地区表现较好\n".format(
            province_avg.index[0]
        ))
...
def create_additional_visualizations(results, output_dir):
    ...
    yearly_avg = results.groupby('年份').mean()
